...
pub fn point_intersects(a: &Point<f64>, b: &Geometry<f64>) -> bool {
    match *b {
        ...
        Geometry::MultiLineString(ref mls) => mls.0.iter().any(|ls| linestring_intersects_point(ls, a)),
...
pub fn linestring_intersects(a: &LineString<f64>, b: &Geometry<f64>) -> bool {
    match *b {
        ...
        Geometry::LineString(ref ls) => a.intersects(ls),
        Geometry::Polygon(ref poly) => a.intersects(poly),
        Geometry::MultiPoint(ref mp) => mp.0.iter().any(|point| linestring_intersects_point(a, point)),
        Geometry::MultiLineString(ref mls) => mls.0.iter().any(|ls| a.intersects(ls)),
        Geometry::MultiPolygon(ref mp) => {
            mp.0.iter().any(|poly| a.intersects(poly))
        },
        Geometry::GeometryCollection(ref gc) => gc.0.iter().any(|geom| linestring_intersects(a, geom))
    }
}
